...
def cluster_image(img):
    ...
    return images, dframe, dframe_removed, skin_cluster_label, height, width, skin_cluster_row

def mean_colour(images):
    _,_,_,_,_,_, skin_cluster_row = cluster_image(images)
...
def segmented_image(images):
    image, dframe, dframe_removed, skin_cluster_label, height, width, _ = cluster_image(images)
    cluster_label_mat = cluster_matrix(dframe, dframe_removed, skin_cluster_label, height, width)
    final_segment(image, cluster_label_mat)
    cv2.imwrite('image.jpg', image["final_segment"])
    return "image.jpg"

# read in image into openCV
def read_image(img_BGR):
